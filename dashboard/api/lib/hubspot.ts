/**
 * HubSpot query functions for the Growth Dashboard (Vercel serverless edition).
 *
  ...
  try {
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      if (res.status === 429 && attempt < MAX_RETRIES) {
        ...
        console.log(`HubSpot 429 — retrying (attempt=${attempt}, delay=${delay}ms)`);
      ...
      if (!res.ok) {
        ...
        console.error(`HubSpot search error: status=${res.status} body=${errorBody}`);
...
export async function fetchCrossData(fromMs: number, toMs: number): Promise<CrossDataRow[]> {
    ...
    do {
      ...
      for (let attempt = 0; attempt <= 5; attempt++) {
        ...
        if (res.status === 429 && attempt < 5) {
          ...
          console.log(`CrossData 429 — retrying (attempt=${attempt}, delay=${delay}ms, categoria=${label})`);
          await new Promise((r) => setTimeout(r, delay));
          continue;
        }
        ...
        if (!res.ok) {
          const errorBody = await res.text();
          console.error(`CrossData page error: status=${res.status} body=${errorBody} categoria=${label} page=${catPages}`);
          throw new Error(`HubSpot API error: ${res.status}`);
        }
  ...
  console.log(`CrossData fetched: elapsed=${elapsed}ms pages=${totalPages} contacts=${totalContacts} uniqueTuples=${result.length}`);
...
export async function fetchLeadsData(
  ...
  const byCanal: CanalBreakdown[] = ALL_CANALES.map((name, i) => ({
    name,
  ...
  const elapsed = Date.now() - start;
  console.log(`Dashboard data fetched: elapsed=${elapsed}ms total=${total} converted=${converted} canales=${byCanal.length}`);
...
export async function fetchBreakdown(
  ...
  if (dimProp === "categoria_de_venta") {
    ...
    const { results } = await hubspotSearch(buildFilters(), [dimProp], 100);
    const freq = new Map<string, number>();
    for (const c of results) {
  ...
  const items: BreakdownItem[] = itemResults
    ...
    .map((r) => {
      const displayName = dimProp === "canal" ? (CANAL_DISPLAY_NAMES[r.val] || r.val) : r.val;
